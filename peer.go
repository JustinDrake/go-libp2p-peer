...
func (id ID) ExtractEd25519PublicKey() ic.PubKey {
	...
	const edPubKeyLen = 32
	const edPubMulticodec = []byte{0xed, 0x01}
	if decoded.Length != edPubKeyLen+len(edPubMulticodec) {
		return nilPubKey
	}

	// Split prefix
	code, pubKey := mc.SplitPrefix(decoded.Digest)
	...
	var pubKeyArr [edPubKeyLen]byte
	copy(pubKeyArr[:], pubKey)
	return &ic.Ed25519PublicKey{&pubKeyArr}
}

func (id ID) ExtractPublicKey() ic.PubKey {
	var pk ic.PubKey

	// Try extracting ed25519 pubkey
	pk = id.ExtractEd25519PublicKey()
	if pk != nil {
		return pk
	}

	// Try extracting other type of pubkey
	// pk = id.Extract...()
